...
# 실습
# 1. '이름'과 '나이'를 Variable Rounting을 통해 받아서 자기소개
def nameage(request, name, age):
    context = {
        'name': name,
        'age': age
    }
    return render(request, 'pages/nameage.html', context)
# 2. 숫자 2개를 Variable Rounting을 통해 받아 곱셈 결과 보여주기
def mulage(request, age1, age2):
    age = age1 * age2
    context = {
        'age1': age1,
        'age2': age2,
    ...
    return render(request, 'pages/mulage.html', context)
...
def birthday(request):
    today = datetime.now()
    birthday = datetime(today.year, 12, 21)
...
# Static Files
# Image, CSS, HTMLq 통틀어 이야기하는 자원 or 파일
