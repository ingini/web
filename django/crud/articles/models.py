...
# 3. python manage.py migrate #=> 실제 DB(db.sqlite3)에 설계도를 적용 (테이블 생성)
...
# 2. article = Article(title='제목2', content='내용2')
#    article.save()
...
#          Article.objects.get(title='세번째')
#          # unique & not null인 컬럼

# 3. 조건 - Article.objects.filter(titles='세번째') # 복수 (QuerySet)
#    WHERE
